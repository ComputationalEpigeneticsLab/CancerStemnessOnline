...
CytoTRACECS<-function(Expfile,Samplefile,ExpfileName,SamplefileName,method,type,email_address,dir_out,ID_new,RNA_v){
  suppressMessages({
    library(dplyr)
    library(jsonlite)
    library(stringr)
  ...
  tryCatch({
    ...
    ####3. CS score ######
    CytoTRACE_re<-CytoTRACE(as.data.frame(expressionFile))
    score<-CytoTRACE_re$CytoTRACE
    ...
    stopMessage<-"CytoTRACE cannot be calculated"
    result_message <- paste0("{",'"error_attention" :','"',stopMessage,'"}')
    write.table(stopMessage,paste0(dir_out,"/stopMessage_CytoTRACE.txt"),
