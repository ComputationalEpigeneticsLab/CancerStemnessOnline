...
StemnessIndexCS<-function(Expfile,Samplefile,ExpfileName,SamplefileName,method,type,email_address,dir_out,ID_new,gene_pairs_SYMBOL_StemnessIndex){
  suppressMessages({
    library(dplyr)
    library(jsonlite)
    library(stringr)
  ...
  dir_out<-dir_out
  gene_pairs_SYMBOL_StemnessIndex<-gene_pairs_SYMBOL_StemnessIndex
  ...
  tryCatch({
    ...
    ####3. CS score ######
    score<-StemnessIndex(expressionFile,pairs = gene_pairs_SYMBOL_StemnessIndex)
    ...
    stopMessage<-"StemnessIndex cannot be calculated"
    result_message <- paste0("{",'"error_attention" :','"',stopMessage,'"}')
    write.table(stopMessage,paste0(dir_out,"/stopMessage_StemnessIndex.txt"),
