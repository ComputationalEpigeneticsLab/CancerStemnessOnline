  ...
  tryCatch({
    ...
    if((corr_re_table_web=="none") & corr_re_table_download=="none"){
      ...
      if (clinical=="none"){
        ...
        clinical_infor<-read.table(paste0(filePath,"/",clinical),header = T,sep = "\t",as.is=TRUE,quote = "",check.names = F,row.names = 1)
      ...
      score <- score - min(score)
      score <- score / max(score)
      CSscore_CytoTRACE<-data.frame(samplename=colnames(GSVA_geneset),score=score)
      ...
      if (base_method =="mRNAsi"){
        score_compare=score_compare[,c(base_method,"Unsupervised_GSVA")]
      }else if(base_method =="GSVA"){
