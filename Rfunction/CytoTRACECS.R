...
CytoTRACECS<-function(ID_new,filePath,ExpfileName,SamplefileName,innerpath,method,type,email_address){
  suppressMessages({
    library(dplyr)
    library(jsonlite)
    library(stringr)
  ...
  tryCatch({
    ...
    CSscore<-data.frame(samplename=names(score$CytoTRACE),score=unname(score$CytoTRACE))
    ####4. out #####
    ### write cs score file 
