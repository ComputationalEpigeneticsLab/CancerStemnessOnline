...
StemSCCS<-function(Expfile,Samplefile,ExpfileName,SamplefileName,method,type,email_address,dir_out,ID_new,RNA_v,gene_pairs_SYMBOL_StemSC){
  suppressMessages({
    library(dplyr)
    library(jsonlite)
    library(stringr)
  ...
  dir_out<-dir_out
  RNA_v<-RNA_v
  gene_pairs_SYMBOL_StemSC<-gene_pairs_SYMBOL_StemSC
  ...
  tryCatch({
    ...
    stopMessage<-"StemSC cannot be calculated"
    result_message <- paste0("{",'"error_attention" :','"',stopMessage,'"}')
    write.table(stopMessage,paste0(dir_out,"/stopMessage_StemSC.txt"),
