...
StemnessIndexCS<-function(ID_new,filePath,ExpfileName,SamplefileName,innerpath,method,type,email_address){
  suppressMessages({
    library(dplyr)
    library(jsonlite)
    library(stringr)
  ...
  tryCatch({
    ...
    load(paste0(innerpath,"/","gene_pairs_SYMBOL_StemnessIndex.RData"))
    ...
    bar_scatter_re=bar_scatter_plot(SampleFile_use,CSscore,filePath,use_col)
    ### other base infor
    stopMessage<-"no"
