...
for (var i=0;i<methods.length;i++){
	if(methods[i].includes("GSVA")){
		if(mult_re["data_type"]=="count"){
			href="CSidentifyResultMultSingle.jsp?UserTaskID="+UserTaskID+"&Method="+methods[i];
			html_inner=html_inner+'<div class="col-lg-4 "><a  href='+href+'><button class="button_style">'+methods[i]+'</button></a></div>';
		}else{
			href="CSidentifyResultMultBulk.jsp?UserTaskID="+UserTaskID+"&Method="+methods[i];
			html_inner=html_inner+'<div class="col-lg-4 "><a  href='+href+'><button class="button_style">'+methods[i]+'</button></a></div>';
		}
	}else{
...
function bubble_plot_speram(x,y,p,title,div){
	...
	for (var i=0;i<y.length;i++){
		...
		color_line.push('red');
		size.push(getAbsoluteValue(y[i])*30);
		if(p[i]<0.05){
			width.push(getAbsoluteValue(y[i])*30);
		}else{
			width.push(0);
		}
