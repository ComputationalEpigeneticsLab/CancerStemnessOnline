...
if(MethodName.includes("GSVA")){
	webpath="upload/"+UserTaskID+"/GSVA/"+MethodName.substr(5,MethodName.length-1);
}else{
webpath="upload/"+UserTaskID+"/"+MethodName;
}
webpath_f="upload/"+UserTaskID;
...
clinical_re="";
$.ajaxSettings.async = false;
$.getJSON(webpath_f+"/time.txt", "", function(data_re) {
	clinical_re=data_re;
});
...
if(methods_end_re.stopMessage=="no"){
	...
	document.getElementById('download_exp').href ="download_loading.jsp?path="+webpath+"/"+methods_end_re.ExpfileName+"&name="+methods_end_re.ExpfileName;
	document.getElementById('download_sample').href ="download_loading.jsp?path="+webpath+"/"+methods_end_re.SamplefileName+"&name="+methods_end_re.SamplefileName;
	...
	if (methods_end_re.Mutation_error=="no"){
		...
		if(clinical_re[MethodName+"_sample"]=="yes"){
			//Sample_source
		...
		if(methods_end_re.hasOwnProperty("RNA_v")){
			if(methods_end_re["RNA_v"]=="success"){
				...
				rna_v_plot_csscore(rna_v_re,'csscore');
				 ...
				 document.getElementById("RNA_v").innerHTML= "No infor ";
			}
	}
}
